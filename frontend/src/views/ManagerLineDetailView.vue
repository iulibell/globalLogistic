...
import { fetchManagerLineDetail } from '@/api/manager.js'
...
const sectionTitle = computed(() =>
  t('page_profile', 'section_manager_line_detail', pageDictFallback('page_profile', 'section_manager_line_detail', uiLang.value)),
)
const lead = computed(() =>
  t('page_profile', 'lead_manager_line_detail', pageDictFallback('page_profile', 'lead_manager_line_detail', uiLang.value)),
)
...
const loadErrorLabel = computed(() =>
  t('page_profile', 'manager_line_detail_load_error', pageDictFallback('page_profile', 'manager_line_detail_load_error', uiLang.value)),
)
const missingParamsText = computed(() =>
  t(
    'page_profile',
    'manager_line_detail_missing_params',
    pageDictFallback('page_profile', 'manager_line_detail_missing_params', uiLang.value),
  ),
)
const btnBack = computed(() =>
  t('page_profile', 'manager_line_detail_back', pageDictFallback('page_profile', 'manager_line_detail_back', uiLang.value)),
)
...
async function loadDetail() {
  ...
  try {
    const res = await fetchManagerLineDetail(origin, dest)
    if (res.code !== 200) {
...
function goBack() {
  router.push({ name: 'profile-manager-line-list' })
}

watch(
  () => [route.query.origin, route.query.dest],
  () => loadDetail(),
)
...
.map-grid {
  ...
  background:
    ...
    radial-gradient(circle at 25% 20%, rgba(75, 145, 255, 0.24), transparent 50%),
    radial-gradient(circle at 75% 80%, rgba(46, 188, 122, 0.18), transparent 45%),
    #091220;
  background-size:
    28px 28px,
    28px 28px,
...
.map-route {
  ...
  height: 3px;
  border-radius: 99px;
  background: linear-gradient(90deg, #5aa3ff, #2ebc7a);
  box-shadow: 0 0 18px rgba(90, 163, 255, 0.5);
}
...
.info-item {
  border: 1px solid rgba(61, 141, 255, 0.18);
  border-radius: 10px;
  padding: 12px;
